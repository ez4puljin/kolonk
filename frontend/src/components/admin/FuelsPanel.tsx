/**
 * Түлшний төрлүүдийн тохиргоо — жагсаалт + нэмэх/засах цонх.
 *
 * Админ панелийн «Түлш» хуудас ба салбарын тохиргооны «Түлш» таб хоёулаа энэ
 * панелийг ашиглана. `branchId` өгвөл литрийн үнэ тухайн салбарын хүчинтэй
 * үнээр (override байвал) харагдана. Үнэ энд засагдахгүй — зөвхөн «Үнийн
 * өөрчлөлт»-ийн батлагддаг урсгалаар.
 */
import { useMemo, useState } from "react";
import { Droplets, Pencil, Plus, Tag } from "lucide-react";
import { useNavigate } from "react-router-dom";

import { errorMessage } from "../../api/client";
import { useCreateFuelMutation, useFuels, useUpdateFuelMutation } from "../../api/queries/fuels";
import type { Fuel, UUID } from "../../api/types";
import { useCan } from "../../hooks/usePermission";
import { t } from "../../i18n/mn";
import { ACTIVE_STATUS_META, statusMeta } from "../../lib/constants";
import { formatMNT } from "../../lib/format";
import { ChipGroup, FieldLabel, NumberField, TextField } from "../../pages/catalog/_shared";
...
export function FuelsPanel({
  branchId,
  branchName,
}: {
  /** Салбарын тохиргоонд — үнэ тухайн салбарынхаар харагдана. */
  branchId?: UUID;
  branchName?: string;
}) {
  ...
  const fuelsQuery = useFuels({ active_only: false, limit: 200, branch_id: branchId });
  ...
  const columns: Column<Fuel>[] = [
    ...
    {
      key: "price",
      header: branchName ? `${t.fuels.price} · ${branchName}` : t.fuels.price,
      align: "right",
      numeric: true,
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card
        title={branchName ? `${t.fuels.title} · ${branchName}` : t.fuels.title}
        actions={
          <div className="flex flex-wrap gap-2">
            <Button variant="secondary" size="md" icon={<Tag />} onClick={() => navigate("/price-changes")}>
              {t.fuels.priceChanges}
            </Button>
            {canManage ? (
      ...
      >
        <div className="border-b border-line px-4 py-2 text-xs text-ink-soft">
          {branchId ? t.fuels.branchPriceHint : t.fuels.subtitle}
        </div>
        <DataTable
          columns={columns}
          rows={fuels}
      ...
      <Modal
        ...
        onClose={close}
        size="md"
        title={editing ? t.fuels.edit : t.fuels.add}
        ...
        <div className="flex flex-col gap-4">
          ...
          {editing ? (
            <div className="flex flex-col gap-1.5">
              <FieldLabel>{t.fuels.price}</FieldLabel>
              <span className="num text-lg font-bold text-ink">{formatMNT(editing.price_per_liter)}</span>
              <span className="text-xs text-ink-soft">{t.fuels.priceLocked}</span>
            </div>
          ) : (
            <NumberField
              name="fuel-price"
              label={t.fuels.priceInitial}
              value={draft.price_per_liter}
              onChange={(value) => patch({ price_per_liter: value })}
              suffix="₮"
              maxDecimals={2}
              hint={t.fuels.priceHint}
            />
          )}
          <div className="flex flex-col gap-1.5">
            <FieldLabel>{t.fuels.color}</FieldLabel>
