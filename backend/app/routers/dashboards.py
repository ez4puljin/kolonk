...
def _own_branch(user: User, requested: uuid.UUID | None) -> uuid.UUID | None:
    """Хяналтын самбарт харагдах салбар.

    Салбарт харьяалагдсан хэрэглэгч (түгээгч, салбарын менежер) ЗӨВХӨН
    өөрийн салбарыг хардаг — хүссэн салбараа дамжуулсан ч үл хэрэгснэ.
    Бүх салбарын эрхтэй хүн (Нягтлан, Admin) сонгосноо, эс бөгөөс бүгдийг.
    """
    own = getattr(user, "branch_id", None)
    return own if own is not None else requested
...
async def branch_shifts(
    ...
    return await report_service.branch_shift_overview(db, _own_branch(user, None))
...
async def fuel_trend(
    ...
    return await report_service.fuel_mile_trend(
        db, days=days, branch_id=_own_branch(user, branch_id)
    )
